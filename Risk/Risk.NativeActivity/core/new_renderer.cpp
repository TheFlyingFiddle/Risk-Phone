...
GLuint createIBO(size_t count)
{
	...
	GLushort* data = (GLushort*)malloc(sizeof(GLushort) * count * 6);
	GLushort* base = data;
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, count * 6 * sizeof(GLushort), base, GL_STATIC_DRAW);
	free(base);
